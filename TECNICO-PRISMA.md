...
**Vivo (jun-2026):** la página pasó a **ACM** en `/asesor/acm` y `/director/acm` (las viejas `/…/tasaciones` redirigen 308). `app/director/acm/page.tsx` es un wrapper que reusa `AcmModule` de `app/asesor/acm/page.tsx`, así que todo fix aplica a ambos roles. Flujo: se elige propiedad sujeto (manual / link de portal / buscador de cartera) y el backend busca **comparables reales** en `properties` + `roomix_properties` con **filtros duros + embedding** → `match_pct` + checklist (precio fuera del %).
