...
vi.mock("@/lib/supabase/admin", () => ({
  createAdminClient: () => ({
    from: (tabla: string) => ({
      select: () => ({
        eq: () => ({ eq: async () => ({ data: tabla === "gcba_puertas" ? filasPuertas : [] }) }),
      }),
...
vi.mock("@/lib/prefactibilidad/dependencias-supabase", () => ({
  dependenciasSupabase: () => ({ gcba: { parcela: async (smp: string) => (centroidesPorSmp[smp] ? { centroide: centroidesPorSmp[smp] } : null) } }),
}));
...
beforeEach(() => { filasPuertas = [{ smp: "053-050-006", es_esquina: false }]; centroidesPorSmp = {}; analizar.mockClear(); });
