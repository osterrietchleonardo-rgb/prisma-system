...
export default function AsesorConfiguracionPage() {
  ...
  useEffect(() => {
    ...
    if (!g) return
    if (g === "conectado") toast.success("¡Google Calendar conectado! Tus visitas se sincronizarán automáticamente.")
    ...
    else if (g === "no_config") toast.error("La integración con Google aún no está configurada. Avisá al administrador.")
    else if (g === "error") toast.error("Hubo un problema al conectar con Google. Intentá de nuevo.")
    // Limpiar el parámetro de la URL
    window.history.replaceState(null, "", "/asesor/configuracion?tab=integraciones")
        ...
        <TabsContent value="integraciones" className="animate-in fade-in-0 slide-in-from-bottom-2 duration-300">
          <Card className="border-accent/10 bg-card/30 backdrop-blur-md">
            <CardHeader>
              ...
              <CardDescription>
                Conectá tu cuenta de Google para que tus visitas aparezcan automáticamente en tu calendario personal.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              {gcalLoading ? (
                ...
                <div className="space-y-4">
                  ...
                  <div className="text-xs text-muted-foreground space-y-1 border border-accent/10 rounded-lg p-4 bg-background/40">
                    <p>✅ Al <strong>agendar</strong> una visita, se crea el evento en tu Google Calendar.</p>
                    <p>✏️ Al <strong>reprogramar</strong>, el evento se actualiza solo.</p>
                    <p>🗑️ Al <strong>cancelar</strong>, el evento se elimina de tu calendario.</p>
                ...
                <div className="space-y-4">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-xl border border-accent/20 bg-accent/5">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">Conectá tu Google para sincronizar tus visitas.</p>
                      </div>
                    </div>
                  ...
                  <p className="text-xs text-muted-foreground border border-dashed border-border rounded-lg p-3">
                    💡 Es opcional. Si no lo conectás, PRISMA funciona igual; solo que tus visitas no aparecerán en tu Google personal.
                  </p>
                </div>
              )}
            </CardContent>
