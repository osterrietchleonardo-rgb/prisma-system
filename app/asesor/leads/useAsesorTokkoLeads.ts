...
import { NormalizedLead, TokkoLead, normalizeLead } from "@/app/director/leads/components/tokko-leads-utils";
import { createClient } from "@/lib/supabase/client";

// Mismo normalizador que usa el director
...
/** Igual que useTokkoLeads del director, pero filtrado por assigned_agent_id */
export const useAsesorTokkoLeads = () => {
    ...
    try {
      ...
      while (hasMore && currentOffset < totalToLimit) {
        const { data: dbLeadsChunk, error: dbError } = await supabase
          ...
          `)
          // ── Filtro clave: solo los leads del asesor ──
        ...
        if (dbLeadsChunk && dbLeadsChunk.length > 0) {
          const chunkNormalized = dbLeadsChunk
            .map((dbLead) => normalizeDbLead(dbLead))
      ...
      console.error("useAsesorTokkoLeads error:", err.message);
