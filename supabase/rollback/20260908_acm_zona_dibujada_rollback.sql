-- ROLLBACK · acm_match_properties y acm_match_roomix como estaban el 8-sep-2026,
-- ANTES de agregarles p_poligonos. Bajadas con pg_get_functiondef desde produccion.
--
-- Para volver atras: aplicar este archivo entero por Management API (en transaccion).
-- Ojo: las firmas NUEVAS tienen un parametro mas, asi que antes de aplicar esto hay que
-- dropear las nuevas; si no, quedan las dos versiones sobrecargadas y la llamada es ambigua.
-- El drop de cada una esta al pie de este archivo, comentado.

CREATE OR REPLACE FUNCTION public.acm_match_properties(p_agency_id uuid, p_query_embedding text DEFAULT NULL::text, p_operation text DEFAULT 'venta'::text, p_type_patterns text[] DEFAULT '{}'::text[], p_m2 numeric DEFAULT NULL::numeric, p_rooms integer DEFAULT NULL::integer, p_dormitorios integer DEFAULT NULL::integer, p_bathrooms integer DEFAULT NULL::integer, p_antiguedad integer DEFAULT NULL::integer, p_loc_patterns text[] DEFAULT '{}'::text[], p_amenities text[] DEFAULT '{}'::text[], p_exclude_id uuid DEFAULT NULL::uuid, p_exclude_ph boolean DEFAULT false, p_obra text DEFAULT 'off'::text, p_obra_sin_dato boolean DEFAULT true, p_barrio text DEFAULT NULL::text, p_zona_niveles boolean DEFAULT false, p_m2_cubierta boolean DEFAULT false, p_zona_min smallint DEFAULT 50, p_peso_semantica smallint DEFAULT 10, p_limit integer DEFAULT 50)
...
-- Dropear primero las versiones con p_poligonos:
-- drop function if exists public.acm_match_properties(p_agency_id uuid, p_query_embedding text, p_operation text, p_type_patterns text[], p_m2 numeric, p_rooms integer, p_dormitorios integer, p_bathrooms integer, p_antiguedad integer, p_loc_patterns text[], p_amenities text[], p_exclude_id uuid, p_exclude_ph boolean, p_obra text, p_obra_sin_dato boolean, p_barrio text, p_zona_niveles boolean, p_m2_cubierta boolean, p_zona_min smallint, p_peso_semantica smallint, p_limit integer, text[]);
-- drop function if exists public.acm_match_roomix(p_query_embedding text, p_operation text, p_type_patterns text[], p_m2 numeric, p_rooms integer, p_dormitorios integer, p_bathrooms integer, p_antiguedad integer, p_loc_patterns text[], p_amenities text[], p_exclude_ph boolean, p_obra text, p_obra_sin_dato boolean, p_barrio text, p_zona_niveles boolean, p_m2_cubierta boolean, p_dedup boolean, p_excluir_sujeto boolean, p_zona_min smallint, p_peso_semantica smallint, p_limit integer, p_cochera boolean, p_piso integer, p_orientacion text, p_disposicion text, p_cochera_patron text, text[]);
