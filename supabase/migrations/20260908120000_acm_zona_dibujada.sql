...
--   · REEMPLAZA al gate de barrio (no se suma: sumarlos da cero comparables apenas la zona
--     cruce dos barrios, que es justo lo que un asesor dibuja)
--   · la dimensión "zona" puntúa 100 con peso 20, igual que hoy puntúa el mismo barrio, así
--     los % siguen siendo comparables con los ACM ya hechos
-- Cuando NO viene, no cambia absolutamente nada.
--
-- La condición se escribe  point(lng,lat) <@ any(p_poligonos::polygon[])  A PROPÓSITO: medido
-- el 7-sep-2026 sobre las 3 zonas reales guardadas, esa forma resuelve por Bitmap Heap Scan en
-- 30 ms; escrita como  exists (select 1 from unnest(...) ...)  el índice no entra (184 ms).
--
-- En acm_match_roomix el camino del dibujo REUSA la rama de abajo (la que va directo a
-- mercado_avisos), no agrega una tercera. Ojo con un detalle de esa rama: hoy la zona pesa 0
-- pero se MUESTRA 100; para el camino del dibujo hay que forzar el peso 20, si no el % sale
-- distinto del decidido.
--
-- El parámetro va al FINAL de cada firma para no correr ninguno de los existentes. Como
-- agregar un parámetro NO reemplaza la función sino que la sobrecarga, primero va el drop de
-- la vieja: con las dos vivas, la llamada de PostgREST quedaría ambigua.
--
-- Se re-otorgan los grants: un drop se lleva puesto el ACL (hoy: PUBLIC + anon + authenticated
-- + service_role, todos con EXECUTE).
--
-- Aplicado por Management API, en transacción.
-- Rollback: supabase/rollback/20260908_acm_zona_dibujada_rollback.sql
...
CREATE OR REPLACE FUNCTION public.acm_match_properties(p_agency_id uuid, p_query_embedding text DEFAULT NULL::text, p_operation text DEFAULT 'venta'::text, p_type_patterns text[] DEFAULT '{}'::text[], p_m2 numeric DEFAULT NULL::numeric, p_rooms integer DEFAULT NULL::integer, p_dormitorios integer DEFAULT NULL::integer, p_bathrooms integer DEFAULT NULL::integer, p_antiguedad integer DEFAULT NULL::integer, p_loc_patterns text[] DEFAULT '{}'::text[], p_amenities text[] DEFAULT '{}'::text[], p_exclude_id uuid DEFAULT NULL::uuid, p_exclude_ph boolean DEFAULT false, p_obra text DEFAULT 'off'::text, p_obra_sin_dato boolean DEFAULT true, p_barrio text DEFAULT NULL::text, p_zona_niveles boolean DEFAULT false, p_m2_cubierta boolean DEFAULT false, p_zona_min smallint DEFAULT 50, p_peso_semantica smallint DEFAULT 10, p_limit integer DEFAULT 50, p_poligonos text[] DEFAULT NULL::text[])
 RETURNS TABLE(id uuid, match_pct integer, sc_zona integer, sc_superficie integer, sc_ambientes integer, sc_dormitorios integer, sc_banos integer, sc_antiguedad integer, sc_amenities integer, sc_semantica integer, cand_m2 numeric, cand_amb integer, cand_dorm integer, cand_ant integer)
 LANGUAGE plpgsql
 STABLE
AS $function$
declare
  ...
  -- criterio del Buscador IA (el dibujo reemplaza al barrio guardado); sumarlos daria cero
  -- comparables apenas la zona cruce dos barrios, que es justo lo que un asesor dibuja.
...
begin
  ...
  cand as (
    ...
    where p.agency_id = p_agency_id
      and p.is_active
      -- Recorte por el dibujo. Se escribe  point(lng,lat) <@ any(...)  A PROPOSITO: es la
      -- unica forma medida que engancha el indice geografico (30 ms contra 184 ms).
...
grant execute on function public.acm_match_properties(uuid, text, text, text[], numeric, integer, integer, integer, integer, text[], text[], uuid, boolean, text, boolean, text, boolean, boolean, smallint, smallint, integer, text[]) to anon, authenticated, service_role;


drop function if exists public.acm_match_roomix(text, text, text[], numeric, integer, integer, integer, integer, text[], text[], boolean, text, boolean, text, boolean, boolean, boolean, boolean, smallint, smallint, integer, boolean, integer, text, text, text);

CREATE OR REPLACE FUNCTION public.acm_match_roomix(p_query_embedding text DEFAULT NULL::text, p_operation text DEFAULT 'venta'::text, p_type_patterns text[] DEFAULT '{}'::text[], p_m2 numeric DEFAULT NULL::numeric, p_rooms integer DEFAULT NULL::integer, p_dormitorios integer DEFAULT NULL::integer, p_bathrooms integer DEFAULT NULL::integer, p_antiguedad integer DEFAULT NULL::integer, p_loc_patterns text[] DEFAULT '{}'::text[], p_amenities text[] DEFAULT '{}'::text[], p_exclude_ph boolean DEFAULT false, p_obra text DEFAULT 'off'::text, p_obra_sin_dato boolean DEFAULT true, p_barrio text DEFAULT NULL::text, p_zona_niveles boolean DEFAULT false, p_m2_cubierta boolean DEFAULT false, p_dedup boolean DEFAULT false, p_excluir_sujeto boolean DEFAULT false, p_zona_min smallint DEFAULT 50, p_peso_semantica smallint DEFAULT 10, p_limit integer DEFAULT 50, p_cochera boolean DEFAULT NULL::boolean, p_piso integer DEFAULT NULL::integer, p_orientacion text DEFAULT NULL::text, p_disposicion text DEFAULT NULL::text, p_cochera_patron text DEFAULT NULL::text, p_poligonos text[] DEFAULT NULL::text[])
...
begin
  ...
  if v_usar_zonas then
    ...
    with cand as (
      ...
      where r.embedding is not null
        -- Recorte por el dibujo. MISMA expresion que indexa mercado_avisos_punto_idx
        -- (gist(point(lng,lat)) where estado='activo' and calidad='ok'), que es exactamente
        -- el filtro de dos lineas mas abajo. Escrito de otra forma, el indice no entra.
...
grant execute on function public.acm_match_roomix(text, text, text[], numeric, integer, integer, integer, integer, text[], text[], boolean, text, boolean, text, boolean, boolean, boolean, boolean, smallint, smallint, integer, boolean, integer, text, text, text, text[]) to anon, authenticated, service_role;
