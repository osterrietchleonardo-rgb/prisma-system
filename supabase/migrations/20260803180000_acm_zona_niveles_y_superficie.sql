-- ACM v5: zona en niveles + superficie comparable + deduplicación.
--
-- Tres arreglos, todos detrás de un flag que por defecto deja el comportamiento IDÉNTICO
-- al actual (así producción no cambia hasta que la app los active al mergear):
--
-- 1) p_m2_cubierta  → compara CUBIERTOS contra CUBIERTOS.
--    Hoy el sujeto entra con sus m² cubiertos (lib/acm/subject.ts: cubiertos+semicubiertos)
--    pero el candidato se filtra por su superficie TOTAL (roomix.area_m2 = total_area_m2,
--    incluye terreno/descubierto). Medido en Belgrano, casas en venta: de 156 candidatas,
--    19 casas reales quedaban afuera solo por eso (ej. 405 m² totales / 246 cubiertos).
--    En toda la red, 2 de cada 3 casas con dato de cubierta tienen el total 15%+ más grande.
--
-- 2) p_zona_niveles → la zona deja de ser un gate binario y pasa a puntuar en 3 niveles:
--       100 = mismo barrio exacto
--        70 = sub-barrio / hermano (Belgrano ↔ Belgrano R, Palermo Soho ↔ Palermo Hollywood)
--        50 = barrio limítrofe (Belgrano ↔ Núñez)
--    Por qué: el gate de hoy es `like '%belgrano%'`, que mete en la misma bolsa a Belgrano
--    (mediana US$3.311/m²), Belgrano R (3.047), Belgrano C (3.121) y Belgrano Chico (4.190)
--    —un spread del 37%— y al mismo tiempo deja afuera a Núñez (3.571, apenas 8% de
--    diferencia) aunque roomix etiquete la MISMA cuadra a veces Belgrano y a veces Núñez.
--
-- 3) p_dedup / p_excluir_sujeto → un comparable por aviso repetido, y la propiedad base
--    deja de aparecer como comparable de sí misma.
--
-- Si el barrio del sujeto no se puede resolver (texto libre que no matchea ningún barrio
-- conocido), se cae al gate viejo por patrones: nunca se queda sin comparables.


-- ── Mapa de barrios relacionados ──
-- Derivado de los datos, no escrito a mano: se calcula con las coordenadas de los avisos
-- de roomix (99,3% las tiene). Dos barrios son limítrofes si comparten celdas de ~300 m
-- Y sus centroides están a menos de 3 km. Umbral calibrado contra pares conocidos:
--   vecinos reales  → Núñez/Saavedra 1,44 km · Belgrano/Núñez 2,00 · Colegiales/Palermo 2,64
--   NO vecinos      → Belgrano/Palermo 3,67 km · Belgrano/Recoleta 6,16
CREATE TABLE IF NOT EXISTS public.acm_barrio_relacion (
  barrio       text    NOT NULL,   -- clave normalizada (acm_norm)
  relacionado  text    NOT NULL,   -- clave normalizada
  zona_score   smallint NOT NULL,  -- 70 = hermano/sub-barrio · 50 = limítrofe
  PRIMARY KEY (barrio, relacionado)
);

CREATE INDEX IF NOT EXISTS acm_barrio_relacion_barrio_idx ON public.acm_barrio_relacion (barrio);

ALTER TABLE public.acm_barrio_relacion ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS acm_barrio_relacion_lectura ON public.acm_barrio_relacion;
CREATE POLICY acm_barrio_relacion_lectura ON public.acm_barrio_relacion
  FOR SELECT TO authenticated USING (true);


-- Recalcula el mapa desde roomix_properties. Re-ejecutable después de cada crawl.
CREATE OR REPLACE FUNCTION public.acm_refrescar_barrios_relacion()
RETURNS integer
LANGUAGE plpgsql AS $fn$
declare v_filas integer;
begin
  -- Padre/hijo: roomix ya guarda la jerarquía (neighborhood='Palermo Soho', city='Palermo').
  -- OJO: muchísimas filas traen city='Capital Federal' / 'Buenos Aires' / 'Comuna N'. Si se
  -- toman como padre, TODOS los barrios de CABA quedan hermanos entre sí (Belgrano hermano
  -- de Barracas). Por eso se descartan los contenedores genéricos, y más abajo los hermanos
  -- tienen que pasar además el control de distancia.
  create temp table _padres on commit drop as
  select distinct
    public.acm_norm(btrim(neighborhood)) as hijo,
    public.acm_norm(btrim(city))         as padre
  from public.roomix_properties
  where neighborhood is not null and city is not null
    and public.acm_norm(btrim(neighborhood)) <> public.acm_norm(btrim(city))
    and btrim(neighborhood) <> '' and btrim(city) <> ''
    and public.acm_norm(btrim(city)) not in (
      'capital federal', 'buenos aires', 'ciudad de buenos aires', 'caba', 'argentina',
      'gba', 'g.b.a. zona norte', 'g.b.a. zona sur', 'g.b.a. zona oeste', 'provincia de buenos aires')
    and public.acm_norm(btrim(city)) not like 'comuna %';

  -- Centroide y celdas de ~300 m por barrio (solo AMBA, que es donde el dato es denso).
  create temp table _pts on commit drop as
  select public.acm_norm(btrim(coalesce(neighborhood, city))) as b, lat, lng,
         floor(lat/0.003) as gy, floor(lng/0.003) as gx
  from public.roomix_properties
  where lat is not null and lng is not null
    and coalesce(neighborhood, city) is not null
    and btrim(coalesce(neighborhood, city)) <> ''
    and lat between -35.1 and -34.2 and lng between -58.9 and -58.2;

  create temp table _cent on commit drop as
  select b,
         percentile_cont(0.5) within group (order by lat) as la,
         percentile_cont(0.5) within group (order by lng) as lo
  from _pts group by b having count(*) >= 40;

  create temp table _cells on commit drop as select distinct b, gy, gx from _pts;

  delete from public.acm_barrio_relacion;

  -- Vecinos limítrofes (50) y hermanos/sub-barrios (70). El hermano pisa al vecino.
  with pares as (
    select a.b as b1, c.b as b2, count(*) as celdas
    from _cells a
    join _cells c on a.gy = c.gy and a.gx = c.gx and a.b < c.b
    group by 1, 2
    having count(*) >= 4
  ),
  limitrofes as (
    select p.b1, p.b2
    from pares p
    join _cent x on x.b = p.b1
    join _cent y on y.b = p.b2
    -- equirectangular: 1° lat ≈ 110,57 km · 1° lng ≈ 91,6 km a la latitud de Buenos Aires
    where sqrt(power((x.la - y.la) * 110.57, 2) + power((x.lo - y.lo) * 91.6, 2)) <= 3.0
  ),
  -- Hermanos: comparten padre. Padre/hijo: relación directa. En los dos casos se exige
  -- cercanía real (≤2,5 km entre centroides) para no arrastrar falsos parentescos.
  hermanos_crudo as (
    select a.hijo as b1, b.hijo as b2 from _padres a join _padres b on a.padre = b.padre and a.hijo < b.hijo
    union
    select least(hijo, padre), greatest(hijo, padre) from _padres
  ),
  hermanos as (
    select h.b1, h.b2
    from hermanos_crudo h
    join _cent x on x.b = h.b1
    join _cent y on y.b = h.b2
    where sqrt(power((x.la - y.la) * 110.57, 2) + power((x.lo - y.lo) * 91.6, 2)) <= 2.5
  ),
  todo as (
    select b1, b2, 70 as score from hermanos
    union all
    select b1, b2, 50 from limitrofes
  ),
  mejor as (
    select b1, b2, max(score) as score from todo where b1 <> b2 group by 1, 2
  )
  -- Se guarda simétrica (a→b y b→a) para poder leerla con un solo índice.
  insert into public.acm_barrio_relacion (barrio, relacionado, zona_score)
  select b1, b2, score from mejor
  union all
  select b2, b1, score from mejor
  on conflict (barrio, relacionado) do update set zona_score = excluded.zona_score;
...
drop function if exists public.acm_match_properties(uuid, text, text, text[], numeric, integer, integer, integer, integer, text[], text[], uuid, boolean, text, boolean, integer);
drop function if exists public.acm_match_roomix(text, text, text[], numeric, integer, integer, integer, integer, text[], text[], boolean, text, boolean, integer);
...
-- Primera carga del mapa de barrios.
SELECT public.acm_refrescar_barrios_relacion();
