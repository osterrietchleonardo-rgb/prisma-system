...
-- ────────────────── 2. acm_match_roomix v2 + índice de sub-barrio ──────────────────
-- Método: cuerpo VIVO transformado con reemplazos verificados (patch-fase2-acm.mjs).
-- Validado el 3-sep contra producción con función de prueba (luego borrada):
--  · PARIDAD: params nuevos en null → mismos 50 ids y pcts que la función viva (la única
--    diferencia observada fue un intercambio entre dos empatados exactos en 97%).
--  · Sub-barrio propio: sujeto 'belgrano r' → 13 avisos de Belgrano R con sc_zona=100.
--  · Cocheras (peso 10): defensa por texto obligatoria (30% de los cocheras=0 mencionan
--    cochera; 12 de los top-50 rescatados por texto).
--  · Piso (6): 100 exacto / 50 a ±2 / 0 lejos / null sin dato. Orientación (5): rosa de los
--    vientos circular, 50 al adyacente. Disposición (5): igualdad estricta.
--  · La COTA SUPERIOR del pool suma los pesos nuevos (piso/ori/disp con score real desde
--    cand; cocheras al tope porque su score lee texto y vive en con_sem).
--  · EXPLAIN: mercado_avisos_zona_sub_idx engancha en ambas particiones (2,4 ms).
--  · Palermo + embedding + todas las dimensiones + dedup: 5,8 s (presupuesto 25 s).
-- Cambia el RETURNS → DROP + CREATE en la misma transacción, timeout y grants re-aplicados.
...
drop function public.acm_match_roomix(text, text, text[], numeric, integer, integer, integer, integer, text[], text[], boolean, text, boolean, text, boolean, boolean, boolean, boolean, smallint, smallint, integer);
...
alter function public.acm_match_roomix(text, text, text[], numeric, integer, integer, integer, integer, text[], text[], boolean, text, boolean, text, boolean, boolean, boolean, boolean, smallint, smallint, integer, boolean, integer, text, text, text)
