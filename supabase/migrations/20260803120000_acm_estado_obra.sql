-- ACM v4: gate de ESTADO DE OBRA (a estrenar / en pozo / usada).
--
-- Por qué: una propiedad a estrenar vale bastante más por m² que una usada del mismo barrio,
-- así que mezclarlas distorsiona el ACM. Medido sobre roomix (venta, USD, mediana US$/m²):
--   Palermo 3.448 vs 2.642 (+31%) · Belgrano 3.495 vs 2.692 (+30%) · Caballito 2.758 vs 2.033 (+36%)
--   Villa Urquiza 2.657 vs 2.258 (+18%) · Almagro 2.461 vs 1.885 (+31%)
--
-- Cómo viene el dato (verificado en la base):
--   properties       → tokko_data.age:      0 = a estrenar · -1 = en pozo/construcción · >0 = años
--   roomix_properties→ property_age_years:  0 = a estrenar · NULL = sin dato · >0 = años
--   La red NO distingue "en pozo" (0 negativos en 134.561 filas): el extractor mapea
--   "a estrenar"/"nuevo" → 0. Por eso acá el pozo de la red se trata igual que a estrenar.
--
-- Agrega dos parámetros; el default ('off', sin filtro) deja el comportamiento IDÉNTICO al actual.
-- El resto del cuerpo es idéntico a 20260724120000_acm_exclude_ph.sql.


-- ── Gate reusable ──
-- Devuelve true si un candidato con antigüedad p_anios entra para un sujeto en estado p_obra.
--   p_anios: NULL = sin dato · 0 = a estrenar · <0 = en pozo · >0 = usada
--   p_obra:  'off' (sin filtro) | 'usada' | 'estrenar' | 'pozo' | 'estrenar_pozo'
--   p_sin_dato: si los "sin dato" entran cuando el sujeto es nuevo (true en alquiler: la red
--               no carga antigüedad en el 99,8% de los avisos de alquiler).
--   p_pozo_es_estrenar: true para la red, que no puede distinguir pozo de a estrenar.
-- Sujeto usado: los "sin dato" SIEMPRE entran (su mediana US$/m² se comporta como la de usadas).
CREATE OR REPLACE FUNCTION public.acm_pasa_obra(
  p_anios integer,
  p_obra text,
  p_sin_dato boolean,
  p_pozo_es_estrenar boolean
)
RETURNS boolean
LANGUAGE sql IMMUTABLE AS $fn$
  select case
    when coalesce(p_obra, 'off') = 'off' then true
    when p_obra = 'usada'   then (p_anios is null or p_anios > 0)
    when p_anios is null    then coalesce(p_sin_dato, true)
    when p_pozo_es_estrenar then p_anios <= 0
    when p_anios = 0        then p_obra in ('estrenar', 'estrenar_pozo')
    when p_anios < 0        then p_obra in ('pozo', 'estrenar_pozo')
    else false
  end
$fn$;


-- Cambia la firma (agrega p_obra y p_obra_sin_dato) → drop de las versiones actuales primero.
drop function if exists public.acm_match_properties(uuid, text, text, text[], numeric, integer, integer, integer, integer, text[], text[], uuid, boolean, integer);
drop function if exists public.acm_match_roomix(text, text, text[], numeric, integer, integer, integer, integer, text[], text[], boolean, integer);
...
begin
  ...
  with cand as (
    select
      ...
      p.bathrooms as ban,
      -- Acepta el -1 de Tokko ("en pozo"), que antes se perdía por el regex de solo positivos.
    ...
    where p.agency_id = p_agency_id
      ...
      -- GATE de ESTADO DE OBRA: a estrenar solo con a estrenar, usada solo con usada.
...
begin
  ...
  with cand as (
    ...
    where r.embedding is not null
      ...
      -- GATE de ESTADO DE OBRA. pozo_es_estrenar = true: la red no distingue pozo de a estrenar.
