...
async function enviarPlantillaEquipo(
  ...
  // 17/9: el transporte (plantilla aprobada, Meta primero, Evolution con `components`) se mudó
  // a lib/whatsapp/enviar-plantilla.ts, porque el chat web necesita lo mismo para escribirle al
  // visitante. Acá queda solo lo propio del aviso al equipo.
  const r = await enviarPlantillaAlTelefono(
    db,
    agencyId,
    { telefono: aviso.destinatario.phone, plantilla: aviso.plantilla, variables: aviso.variables },
    fetchFn
  )
  return {
    resultado: r.resultado as ResultadoCanal,
    wamid: r.wamid,
