# GUÍA DEL ASESOR — SISTEMA PRISMA
...
1. [Primeros pasos: cómo entrar](#1-primeros-pasos-cómo-entrar)
2. [Cómo moverte por PRISMA](#2-cómo-moverte-por-prisma)
3. [Mi Dashboard](#3-mi-dashboard)
4. [Pulso de Mercado](#4-pulso-de-mercado)
5. [Mi Pipeline (tablero de leads)](#5-mi-pipeline-tablero-de-leads)
6. [Mis Propiedades](#6-mis-propiedades)
7. [Tracking Performance (mi actividad diaria)](#7-tracking-performance-mi-actividad-diaria)
8. [Leads Tokko](#8-leads-tokko)
9. [WhatsApp Bandeja](#9-whatsapp-bandeja)
10. [Leads WhatsApp](#10-leads-whatsapp)
11. [Marketing IA](#11-marketing-ia)
12. [Contratos IA](#12-contratos-ia)
...
**Qué es:** el tablero del mercado inmobiliario real argentino.
...
1. Mirá la columna **"Nuevo"**: son leads que todavía no trabajaste.
2. **Arrastrá** una tarjeta de una columna a otra a medida que avanza la conversación.
3. Hacé clic en una tarjeta para ver el detalle del lead.

> 💡 **Tip:** mantené el tablero al día. El pipeline ordenado es la mejor forma de no perder oportunidades y de que tu director vea tu trabajo.
> ⚠️ Solo ves los leads **asignados a vos**.

---

## 6. Mis Propiedades

**Qué es:** el catálogo de propiedades que vos manejás (captaciones propias o asignadas por el director).
...
## 7. Tracking Performance (mi actividad diaria)
...
- **Lista** es lo de siempre: el historial de actividades, una fila por cada cosa que cargaste. Ahí también aparece el cartelito de proceso (VENDEDOR, COMPRADOR, LOCADOR, LOCATARIO o SIN DEFINIR), al lado del tipo de actividad, para que sepas de qué se trataba esa fila sin tener que abrirla.
- **Pipeline** es un **tablero**: casi siempre cada cliente aparece **una sola vez**, como una tarjeta parada en la etapa en la que está hoy — salvo cuando le estás gestionando más de una cosa a la vez (por ejemplo le buscás una propiedad y además le vendés la suya, o le vendés su casa y le alquilás otra), que ahí tiene una tarjeta por cada cosa (más abajo, en *"Un cliente con más de un proceso abierto"*). Las columnas son las 6 etapas: Prospección, Prelisting, Prebuying, Captación, Reserva y Cierre.

**Cómo leerlo:**
- Cada tarjeta te muestra el nombre del cliente, su celular, la propiedad y cuántas actividades le cargaste.
- Arriba de cada columna hay un **número**: cuántas tarjetas tenés en esa etapa. Para casi todos tus clientes es lo mismo que "cuántos clientes", pero si alguno tiene más de un proceso abierto, cuenta una vez por cada tarjeta suya en esa etapa. De un vistazo ves dónde se te está juntando el trabajo.
...
- Cada tarjeta tiene un **cartelito de color** que te dice de qué lado del negocio es, y el texto te dice exactamente cuál: **VENDEDOR** o **LOCADOR** (índigo) si el cliente tiene una propiedad y te la está ofreciendo, **COMPRADOR** o **LOCATARIO** (violeta) si le estás buscando una, y **SIN DEFINIR** (ámbar) en tarjetas viejas que todavía no tienen ese dato cargado.

> ⚠️ **¿Qué es una tarjeta SIN DEFINIR?** Es una actividad vieja, cargada antes de que el sistema empezara a preguntar de qué proceso se trataba. Hoy quedan muy pocas. Se mueve por el tablero sin ningún problema, igual que las demás. Para dejarla prolija: abrí la tarjeta y en el cartel de arriba vas a ver botones con las opciones que le sirven a **todas** sus actividades viejas — tocás una y queda clasificada, sin marcarla como modificada ni tocarle nada más. Si esa tarjeta mezcla actividades de los dos lados del negocio (por ejemplo una captación y una búsqueda que quedaron pegadas en la misma tarjeta sin proceso), no va a haber ningún botón: el cartel te va a avisar que hay que resolverla actividad por actividad desde la vista Lista.
...
**Ver todo lo que pasó con un cliente:** tocá la tarjeta (en el nombre) y se abre un panel lateral con **toda la historia de esa tarjeta** en orden: cada actividad que cargaste y cada vez que la moviste de etapa, con la fecha. Desde ahí también podés editar cualquier actividad. Si el cliente tiene otros procesos abiertos, esto es la historia completa de este — para ver los demás abrís cada tarjeta por separado.
...
#### Un cliente con más de un proceso abierto

Pasa más seguido de lo que parece: le estás buscando una propiedad a alguien y, en el medio, esa misma persona te pide que le gestiones la venta de la suya. O le vendés su casa y encima le andás buscando algo para alquilar mientras se muda. Antes, el tablero solo podía mostrarlo en un lugar — apenas le cargabas la actividad nueva, la tarjeta anterior desaparecía de donde estaba. Ahora no: cada proceso tiene su propia tarjeta y avanza solo.

El proceso ya no es sólo "compra o venta": son cuatro opciones, porque la agencia también maneja alquileres. Un cliente puede ser **Vendedor** (tiene una propiedad y la vende), **Locador** (tiene una propiedad y la alquila), **Comprador** (busca comprar) o **Locatario** (busca alquilar). Vendedor y Locador son las dos caras de "ofrece una propiedad"; Comprador y Locatario son las dos caras de "busca una propiedad" — por eso comparten color de cartelito.

- **Ahora te lo pregunta en las 6 etapas.** Antes, en Prelisting, Captación y Prebuying el sistema lo completaba solo porque sólo había una opción posible. Ya no: una Prelisting o una Captación pueden ser de un Vendedor o de un Locador, y una Prebuying puede ser de un Comprador o de un Locatario, así que en esas etapas también te pregunta cuál de las dos es. En Prospección, Reserva y Cierre sigue siendo obligatorio elegir entre las cuatro, sin ninguna marcada de entrada. El campo sólo viene relleno y bloqueado cuando ya lo decidiste vos antes, arrastrando una tarjeta o abriendo un proceso nuevo desde su ficha (los dos casos de abajo).
- **Cómo se abre un proceso nuevo:** desde **"Nueva Actividad"**, elegís el mismo cliente y cargás la actividad que te falta (por ejemplo, un Prelisting si ya lo tenías como Comprador en Prebuying). O más directo: abrís la tarjeta que ya tiene, y en el panel te aparecen botones **"Abrir proceso de…"** con las opciones que ese cliente todavía no tiene (hasta tres, una por cada proceso que le falte). Tocás una, se abre el formulario con el cliente ya cargado y arranca directo en Prelisting si ofrece una propiedad, o en Prebuying si busca una (después lo podés cambiar de etapa igual que cualquier otra tarjeta). **La tarjeta que ya tenías no se mueve** al hacer esto.
- **Al arrastrar:** una tarjeta de Comprador o Locatario no se puede soltar en Prelisting ni en Captación (son de quien ofrece una propiedad); una de Vendedor o Locador no entra en Prebuying. Si lo intentás, la tarjeta vuelve sola a su lugar y te avisa por qué. En el menú de los tres puntitos, esas etapas ya aparecen apagadas para que ni lo intentes. Las tarjetas SIN DEFINIR no tienen esta restricción: se mueven a cualquier columna, como siempre.
- **En la ficha del cliente:** si tiene otros procesos abiertos, arriba te avisa cuáles (por ejemplo, "También tiene un proceso de Locador abierto"). El historial que ves ahí es el de esa tarjeta puntual — para ver los demás, abrís cada tarjeta por separado.

> 💡 **Se sacó la pregunta "Tipo de Lead" de Prospección.** Antes, al cargar una Prospección te preguntaba dos veces casi lo mismo: "Proceso" y, más abajo, "Tipo de Lead" (con las mismas cuatro opciones). Se unificaron en una sola: ahora "Proceso" cubre eso también, y "Tipo de Lead" ya no aparece en el formulario. Lo que ya habías cargado con esa pregunta no se borró, sigue viéndose si abrís el detalle de esa fila en la Lista.
...
**Qué es:** tu bandeja de entrada de los chats de WhatsApp que tenés asignados.
...
**Qué es:** los leads que se capturaron a través de WhatsApp (separados del listado de Tokko).
...
1. Andá a **"Nuevo Contrato"** y elegí el tipo (Locación, Boleto de compraventa, Reserva, etc.).
2. Completá los campos del cliente y la propiedad en el asistente.
3. Guardás y **descargás el PDF** (o lo ves desde la tabla "Mis Contratos").
4. Al **finalizar**, el contrato queda **pendiente de firma**.

> 💡 **Tip:** la firma es **presencial (en papel)**. El PDF queda listo para imprimir y firmar.
> 💰 Finalizar/generar el contrato consume 5 créditos.
> 💡 Si necesitás cambiar un contrato, podés **modificarlo** (te pide un motivo) o **eliminarlo** (también con motivo). Tené presente que el director ve esa trazabilidad.

---

## 13. Mi Calendario (visitas)

**Qué es:** tu agenda de visitas a propiedades. Ves **solo tus visitas**.
...
- Si la visita ya pasó, no podés modificarla (aparece "Esta visita ya no puede ser modificada").

> 💡 **Tip:** cargá objeciones e intereses al agendar. Esa info te sirve para preparar la visita y para que el análisis comercial sea más útil.

---

## 14. Tutor IA

**Qué es:** un mentor con IA que responde tus preguntas usando los **documentos y manuales** que la agencia subió a la Biblioteca.

**Cómo usarlo:**
1. Escribí tu pregunta (ej.: "¿Cómo manejo la objeción de precio en una locación?").
2. El Tutor responde y te muestra las **fuentes** (documentos) que usó.
...
**Qué es:** un buscador conversacional de propiedades. Le hablás como a una persona y te trae propiedades que coinciden.
...
**Qué es:** la base de conocimiento de la agencia: manuales, guiones, instructivos, etc.

**Cómo usarla:**
- Consultá y abrí los documentos que el director compartió para los asesores.
- Es la misma fuente que usa el **Tutor IA** para responderte.
...
**3. Notificaciones**
- **Notificaciones en el celular (Push):** activalas con el botón "Activar Push" para recibir alertas instantáneas en la barra de notificaciones del teléfono.
- **Nuevos Leads:** aviso cuando se te asigna un lead.
- **Recordatorios de Visitas:** aviso 24 hs antes de una visita.

> 💡 **Tip:** para recibir push en el celular, abrí PRISMA desde Chrome o Safari e **instalalo en la pantalla de inicio**.
...
**Qué es:** tu canal directo para enviar **feedback** al equipo de PRISMA (Vakdor).

**Cómo usarlo:** escribí tu idea, problema o sugerencia. El equipo la recibe y le da seguimiento.

> 💡 **Tip:** si algo te traba el trabajo o se te ocurre una mejora, decilo acá. Las sugerencias se revisan.
...
> ⚠️ Vos **no podés recargar créditos**. Si la agencia se queda sin saldo, las herramientas IA se bloquean y la pantalla te avisa que contactes al director.
...
**¿Por qué no veo los leads de mis compañeros?**
Porque como asesor solo ves **tu** información. Es por diseño.

**¿Por qué no puedo configurar WhatsApp?**
La conexión de WhatsApp la administra el **director**. Vos usás la bandeja una vez conectada.

**Se me bloquearon las herramientas de IA, ¿qué hago?**
Probablemente la agencia se quedó sin créditos. Avisale a tu director para que recargue.
