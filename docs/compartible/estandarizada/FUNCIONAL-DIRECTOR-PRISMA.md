...
- **Lista** es lo de siempre: el historial de actividades del equipo, una fila por registro. Ahí también se ve el cartelito de proceso (VENDEDOR, COMPRADOR, LOCADOR, LOCATARIO o SIN DEFINIR) junto al tipo de actividad, así identificás de qué se trataba cada fila sin tener que entrar al Pipeline.
- **Pipeline** es un **tablero**: casi siempre cada cliente aparece **una sola vez**, como una tarjeta parada en la etapa en la que está hoy — salvo cuando un asesor le está gestionando más de una cosa a la vez (compra, venta o alquiler), que ahí tiene una tarjeta por cada una (más abajo, en *"Un mismo cliente en varias columnas a la vez"*). Las columnas son las 6 etapas: Prospección, Prelisting, Prebuying, Captación, Reserva y Cierre.

**Cómo leerlo como director — es la foto del equipo:**
- El número arriba de cada columna te dice **cuántas tarjetas hay en esa etapa**. Para casi todos tus clientes es lo mismo que "cuántos clientes", pero no siempre: uno con más de un proceso abierto pesa una vez por cada tarjeta suya en esa etapa. Igual te sirve para ver el embudo real: si tenés 40 en Prospección y 2 en Reserva, sabés exactamente dónde se está trabando el equipo.
...
- Cada tarjeta tiene un **cartelito de color** que dice de qué lado del negocio es, y el texto dice cuál exactamente: **VENDEDOR** o **LOCADOR** (índigo) si el cliente ofrece una propiedad, **COMPRADOR** o **LOCATARIO** (violeta) si busca una, o **SIN DEFINIR** (ámbar) en las tarjetas viejas que todavía no tienen ese dato.
...
**Trazabilidad por cliente:** al tocar una tarjeta se abre un panel con **toda la historia de esa tarjeta** en orden: cada actividad cargada y cada movimiento de etapa, con fecha y con acceso a editar. Si el cliente tiene otros procesos abiertos, esto es la auditoría completa de este — para ver los demás hay que abrir cada tarjeta por separado.
...
#### Un mismo cliente en varias columnas a la vez

En el tablero te vas a encontrar clientes con **más de una tarjeta** — es un cliente al que un asesor le está gestionando más de un proceso: le busca propiedad para comprar y a la vez le vende la suya, o le vende su casa y de paso le busca algo para alquilar. No es un duplicado ni un error de carga: es el mismo caso de siempre (bastante común, y ahora también con alquileres) que antes el tablero no podía mostrar, porque solo alcanzaba para una tarjeta por cliente. Cada tarjeta avanza por su cuenta, así que vas a ver, por ejemplo, la de VENDEDOR todavía en Prelisting mientras la de COMPRADOR ya llegó a Reserva.

Tampoco te infla los números que ya tenías: el Dashboard y los objetivos mensuales siguen contando **por tipo de actividad**, no por cliente ni por tarjeta, así que un mismo cliente con varias tarjetas no duplica nada en el ranking de un asesor ni en la facturación del equipo.

El **informe de los lunes es distinto y conviene que lo sepas**: ahí cada cliente sigue contando **una sola vez**, en la etapa de su actividad más reciente — ese informe todavía no distingue entre los cuatro procesos como hace el tablero. Entonces un cliente con dos o más procesos abiertos te va a aparecer con **varias tarjetas en el Pipeline** pero **una sola vez** en el informe del lunes. Ninguno de los dos está mal — son dos fotos distintas del mismo cliente —, pero si algún lunes un número no te cierra contra lo que ves en el tablero, es por esto y no por un error de carga.

Como dato de gestión, es una buena señal para mirar en las reuniones: un cliente con más de un proceso abierto tiene **varias oportunidades de cierre** en danza, no una sola. Vale la pena fijarse qué asesores están detectando esos casos y haciéndoles seguimiento a cada proceso, en vez de quedarse solo con el que entró primero.
...
> ⚠️ **Vos también cargás actividad, así que esto te toca a vos también.** El formulario que usás es el mismo que el del asesor, y tuvo tres cambios chicos: ahora te pregunta el proceso en **las seis etapas** (antes, en Prelisting, Captación y Prebuying ya venía resuelto solo); la pregunta "Tipo de Lead" de Prospección **desapareció** (se unificó con la de proceso, no se repite más); y las tarjetas **SIN DEFINIR** del tablero tienen un botón para clasificarlas en un clic. Están explicados con el detalle completo en la guía del asesor, secciones "Mi Pipeline" y "Tracking Performance".
...
> 🤝 **Para coordinar una visita:** las tarjetas de la red muestran el **WhatsApp y el teléfono** de la inmobiliaria que publica, y el botón **"Ver en portal original"** abre ese aviso puntual. Tu equipo contacta al colega sin salir del sistema.
...
> 🔒 **La ficha no expone al colega, por diseño.** No lleva su nombre, ni su logo, ni su teléfono, ni el link al aviso original — sale con la tarjeta de tu asesor y la marca de tu agencia. Esos datos los ve el equipo puertas adentro, en la tarjeta del Buscador, para coordinar la visita.
