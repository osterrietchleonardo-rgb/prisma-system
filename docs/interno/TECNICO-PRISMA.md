...
- **Publicación:** blog → inserta en `blog_posts` de **vakdor-app** (`PROJECT_URL`+`SERVICE_ROLE_SECRET`, `featured_image_url`=portada pública). LinkedIn → Buffer `createPost` (`BUFFER_API_KEY`, canal personal `6a4aca1140483446287320b8`): si la idea es **`carrusel`** con `carousel.pdf`, se publica como **document post** (asset `document{url,title,thumbnailUrl}` — LinkedIn lo muestra deslizable); si no, texto+imagen (`resolverImagenLinkedIn`). Verificado contra la API real de Buffer (draft con `DocumentAsset` mimeType `application/pdf`). El **primer comentario requiere plan pago** de Buffer (por ahora se pega a mano). **Cron** `POST publicar-programadas` (auth `assertCron`) + workflow `.github/workflows/marketing-publish.yml` (cada 30 min) publica todas las `aprobada` con `programada_para` vencida — un solo cron para blog+LinkedIn (imagen o documento), un fallo por idea no corta el resto.
