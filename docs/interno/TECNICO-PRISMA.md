...
- **Reuso en n8n (jul-2026):** el agente n8n `Cartera_Propiedades` (WhatsApp), cuando el lead pega el link de una propiedad publicada, ya **no** usa el `webSearch` de OpenAI (buscador, no scraper → volvía vacío con ML/ZonaProp). Su herramienta `Informacion_Web` es ahora un **HTTP Request Tool** que hace `POST /extract` a este mismo servicio (credencial n8n header-auth `ACM_EXTRACTOR`, header `x-extractor-secret`), pasando la URL ya saneada por el Code node `extraer_agency_id` (`extracted_url`, con strip de puntuación final). ML devuelve datos parciales por JSON-LD (tipo/barrio/amenities, sin precio/ambientes salvo que el extractor tenga `GEMINI_API_KEY` para el fallback IA); alcanza para matchear contra el stock interno por zona+tipo+características. El Code node quedó con `retryOnFail` (3 intentos/2s) para el timeout esporádico del task-runner.
