...
- **Reintento por IP vacía (jul-2026, rama `fix/extractor-reintenta-ip-vacia`):** el link seguía trayendo datos vagos (`metodo:'opengraph'`, precio 0, moneda null, solo título+slug). Diagnóstico en vivo (debug): el proxy rotativo da **~1 de 3 IPs buenas**; con IP quemada ML sirve solo el `<head>` (og:title) **sin cuerpo ni JSON-LD**. Bug: `looksBadPage` NO marcaba esa página como mala (título válido + algo de HTML) → se **aceptaba la página vacía sin reintentar**. Fix en `extract()`: reintenta mientras no haya **contenido real** (JSON-LD o >300 chars de texto), no solo cuando "parece" error; se queda con el mejor intento (`pageScore`); `maxAttempts` 3→5 (~87% de lectura completa); `fetchOnce` devuelve rápido si la IP no trajo nada (sin perder ~7s de networkidle) para reintentar antes; `ok` solo si hay datos sustanciosos (precio o m²) → si solo hay slug, `requiere_completar_manual`. Lógica validada de forma determinista. **No reemplaza** un proxy con saldo/targeting AR: si TODAS las IPs están quemadas, sigue sin poder leer (límite de infra).
