...
- **Publicación:** blog → inserta en `blog_posts` de **vakdor-app** (`PROJECT_URL`+`SERVICE_ROLE_SECRET`, `featured_image_url`=portada pública). LinkedIn → Buffer `createPost` (`BUFFER_API_KEY`, canal personal `6a4aca1140483446287320b8`), texto+imagen (`resolverImagenLinkedIn`); el **primer comentario requiere plan pago** de Buffer (por ahora se pega a mano). **Cron** `POST publicar-programadas` (auth `assertCron`) + workflow `.github/workflows/marketing-publish.yml` (cada 30 min) publica todas las `aprobada` con `programada_para` vencida — un solo cron para blog+LinkedIn, un fallo por idea no corta el resto.
