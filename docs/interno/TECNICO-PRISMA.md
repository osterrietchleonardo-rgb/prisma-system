...
- **Bot IA prendido/apagado por campaña:** al crear, el director elige `bot_active_on_reply` (default `true`). El chat **nuevo** que crea la campaña nace con `wa_conversations.bot_active = campaign.bot_active_on_reply ?? true` (en `processCampaign` y en `sendCampaignMessage` del envío manual puntual). Si el chat **ya existía**, desde el fix de duplicación (jul-2026) `processCampaign` **sí** le sobreescribe `instance_id`, `bot_active` y `clasificacion` con los de la campaña — a propósito, pero pisa la config manual del asesor y la clasificación previa del lead. Caso de uso: campañas de reclutamiento u otros no-clientes → bot apagado para que la IA no responda. El webhook (`/api/webhooks/meta`) ya respeta `bot_active`: si está OFF guarda el mensaje del lead en `n8n_chat_histories` y no dispara n8n.
