...
- `POST /api/marketing-ia/subir-foto` — sube una foto propia (multipart `file`) para la propiedad que todavía no está en Tokko, y devuelve `{ url }`. **0 créditos**: solo deja el archivo en `marketing-images` bajo `fotos-ia/<userId>/subidas/`; el retoque sigue siendo `editar-foto`, que baja la foto de cualquier URL. Valida contra `lib/marketing-ia/subida-foto.ts` (JPG/PNG/WEBP, 15 MB, comparación exacta del tipo) y normaliza con `sharp().rotate().resize(1600, inside).jpeg(92)`. **El `.rotate()` no es opcional**: `traerFoto` de `editar-foto` no lo hace, así que un EXIF que sobreviva termina en una foto retocada acostada. Devuelve 401 —no 500— cuando no hay sesión. Ver LÓGICA §13.7.
...
- **Corrección 11/09/2026:** el `media_id` no se perdió, pero el ARCHIVO sí: Meta lo borra a los **7 días** y la app nunca lo bajaba, así que el chat no tenía nada que reproducir (168 audios y 29 fotos imposibles de abrir). Desde esa fecha el webhook lo baja al llegar (`lib/whatsapp/adjuntos-entrantes.ts`) y lo sube a Storage `documents/wa-inbound/{agency}/{conv}/{media_id}.{ext}`; el link queda en `metadata.media_url` (+ `media_mime`, `media_saved_at`). Corre en paralelo con n8n y se espera con `allSettled` antes de responder; si falla, el mensaje entra igual. Hacía falta además `media-src 'self' blob: https://*.supabase.co` en el CSP de `next.config.mjs`: sin esa directiva el navegador bloqueaba **todo** audio/video externo (también los que manda el asesor). Los adjuntos sin link muestran "ya no se puede escuchar: WhatsApp guarda los archivos solo 7 días".
...
- **Pedidos de asesores de Central (16-sep-2026, rama `feat/acm-pedidos-asesores`):** cuatro cambios, ninguno toca SQL.
  - **Link a la vista:** `ComparableCard` muestra "Ver publicación" (`c.url`) al lado de "Ver checklist" (antes vivía al fondo del checklist desplegado). Las etiquetas del título pasan a `flex-wrap` (en 390 px pisaban el "COMPARABLE" del %).
  - **Laundry:** `Amenidades.laundry?` (opcional: los sujetos guardados no la traen). Token `laundry(?! ?room)` en `lib/acm/subject.ts` — Postgres lo evalúa con `~*` (el ARE admite `(?!`), y excluye "Laundry room", que es como Tokko traduce el lavadero de la unidad. Medido: en la red "Laundry" y "Lavadero" coinciden solo en 3.623 de 30.173 avisos; la expresión encuentra exactamente los 10.387 con "Laundry". También en `tokko.ts`, `extract.ts` y `extractor-server.mjs` (regex + prompt + `AMEN_KEYS`).
  - **Antigüedad visible:** `AcmComparable.antiguedad` (= `cand_ant`), `FichaComparable.antiguedad`, `AcmFichaSnapshot.subject.antiguedad`. Convención de `lib/acm/antiguedad.ts`: 0 = a estrenar, negativo = en pozo, null = sin dato. Las búsquedas guardadas antes la recuperan del `comp_val` del checklist (`antiguedadDesdeChecklist`). En la ficha las specs pasan a 7 columnas (`repeat(6, 1fr) 1.45fr`, para que "USD 3.160/m²" no parta renglón; medido en impresión: cada hoja sigue en 1123 px).
  - **Sumar comparable por link:** `POST /api/acm/comparable-link` (`{url, sujeto, operacion, search_id}` o `{accion:"quitar", id, search_id}`). Lógica en `lib/acm/comparable-link.ts`: (1) Zonaprop → `mercado_avisos` por id (el número del aviso ES el id); activo+ok+venta entra como `source:"roomix"` (`roomix_<id>`), si no como `source:"link"`; (2) otro portal → `extractFromUrl(url, {conFotos:true})`. El % lo calcula `lib/acm/puntaje-link.ts`, réplica de `acm_match_roomix` para un candidato **sin filtros duros y sin semántica** (su peso no se cuenta). Paridad verificada contra la función real sobre 305 avisos de 8 ACM: 305/305 en % y sub-puntajes (hizo falta redondear como `round()` de numeric: 66,5 llegaba como 66,4999). **Si cambian pesos o dimensiones en la SQL, cambiarlos también ahí.** Zona con `acm_barrio_relacion`; si el barrio del sujeto no está en la tabla, mismo lugar = null (como la búsqueda) y otro lugar = 0; si el aviso solo dice "CABA, Argentina", null. Se guarda en `acm_searches.resultados.agregados` (asesor solo en las propias, director en su agencia) y el servidor no guarda uno que la búsqueda ya trajo (`ya_en_lista`).
  - **Fotos del propio aviso (`lib/acm/fotos-aviso.ts`, pura):** Zonaprop por la carpeta `/avisos/1/00/58/05/67/24/` (número del aviso en pares); Argenprop por `/static-content/<número al revés>/`; MercadoLibre solo dentro de `ui-pdp-gallery` hasta la columna siguiente (después vienen fotos de otras unidades). Portal desconocido = sin fotos. El extractor (Tier 2) devuelve `html` solo si se le pide `con_html:true`, para que el filtro viva en un solo lugar. `HOSTS_PORTALES` (argenprop + mlstatic) entra en `HOSTS_RED`, así pasan por `/api/foto-red`. En la ficha, `datosDeLink()` (`lib/acm/ficha.ts`) revalida las fotos de `link_datos` contra `HOSTS_RED`: viajaron por el navegador.
  - ⚠️ **Redeploy del extractor en EasyPanel** para que devuelva `html`; sin eso, los avisos fuera de la red entran sin fotos cuando Tier 1 está bloqueado (en Vercel, casi siempre). **Hecho el 16-sep** (servicio en `98ae1df`, verificado con Argenprop: 5 fotos).
- **Link seguro — el servidor ya no abre direcciones internas (16-sep-2026, rama `fix/acm-barrio-de-los-links`):** `extractFromUrl` hacía `fetch(url, {redirect:"follow"})` con cualquier `http(s)` desde el 25-jun (`4e6ea94`): un usuario logueado podía hacer que el servidor abriera `169.254.169.254`, `127.0.0.1:3000` o un link que redirigía adentro. Decisión de Leonardo: **cualquier host público** (no lista cerrada de portales), todo lo interno rechazado. `lib/acm/url-segura.ts`, mismo patrón que `fotos-url.ts` + `fotos-descarga.ts`:
  - `validarFormaUrl` (pura): solo http/https, sin usuario:clave, sin hosts sin punto (`localhost`, `n8n`) ni sufijos internos, e IP literal pública (`new URL` ya normaliza `2130706433`/`0x7f.1` a `127.0.0.1`). `esIpInterna`: 127/8, 10/8, 172.16/12, 192.168/16, 169.254/16, 100.64/10, 0/8, reservadas y multicast; IPv6 ::, ::1, fc00::/7, fe80::/10, multicast, y las que esconden una IPv4 (`::ffff:`, NAT64, 6to4, Teredo).
  - `validarUrlPublica`: además resuelve el nombre y exige que **todas** las direcciones sean públicas.
  - `crearTransporteNode`: el pedido va con `node:http(s)` y un `lookup` que vuelve a controlar **al conectar** (el `fetch` global no lo permite): cierra el DNS que cambia entre el control y la conexión. Tope 5 MB, descomprime gzip/deflate/br, respeta el charset.
  - `obtenerPaginaSegura`: sigue las redirecciones **a mano** (máx. 5) y pasa las capas en cada salto. Tira `UrlNoPermitida`.
  - En `extract.ts`: se valida al entrar (link interno → `aviso` "Ese link no se puede abrir: …", sin tocar la red ni el Tier 2); un salto interno en Tier 1 corta sin pasar al Tier 2; y `tryExtractorService` valida antes de mandar la URL. `comparable-link.ts` muestra ese aviso tal cual.
  - Pruebas (`url-segura.test.ts`, `extract-url-segura.test.ts`): los cuatro ataques pedidos + un servidor real en 127.0.0.1 que cuenta pedidos (llegan 0, con prueba de control). 10 mutantes muertos, incluido reponer el `fetch` original. Verificado contra internet real: redirectores públicos hacia 169.254.169.254 y 127.0.0.1, y `localtest.me` (resuelve a 127.0.0.1), rechazados; Argenprop abre.
  - **Límite abierto:** el extractor de EasyPanel abre la página con un navegador que sigue solo las redirecciones que haga la página, dentro de la red donde corren n8n y el resto. Ya no recibe links internos, pero una redirección interna hecha por la página no se controla ahí (cambio en `extractor-server.mjs` + redeploy, sin hacer).
- **Barrio de los links (16-sep-2026, misma rama):** en el JSON-LD de Argenprop y Zonaprop `addressLocality` es la ciudad ("CABA, Argentina") y `addressRegion` el barrio. Tier 1 tomaba `addressLocality || addressRegion`. Ahora `fromJsonLd` devuelve los dos como candidatos y `elegirBarrio` (`lib/acm/barrio-aviso.ts`, puerto de `pickBarrioCoherente` del extractor) elige: descarta zonas generales, gana el que confirma el slug del link o repiten más fuentes. Si cambia una de las dos reglas, cambiar la otra. `comparable-link.ts` usa su `ZONA_GENERAL`.
...
- **Navegación (sep-2026, menú agrupado):** la lista de páginas vive en **un solo lugar**, `lib/nav/menu.ts` (29 renglones del director / 23 del asesor, en 9 grupos; `lib/nav/menu.test.ts` fija la lista completa renglón por renglón y falla si alguno cambia). **12/9/2026:** las solapas de "Asesor IA WhatsApp" (Contactos, Plantillas, Campañas, Configuración IA) y las de "Marketing IA" pasaron a ser páginas propias en los grupos «Difusión» (`/{rol}/difusion/*`) y «Marketing IA» (`/{rol}/marketing-ia/*`, rutas en `lib/marketing-ia/rutas.ts`; `/{rol}/marketing-ia` redirige a `crear-anuncio` desde `next.config.mjs`); «Contratos IA» quedó solo en «Documentación». Cada rol ve como páginas las solapas que veía. `menuPara(rol, { agencyId })` saca Contratos IA (y el grupo, que queda vacío) para la agencia con el módulo desactivado (`lib/access/contratos-ia.ts`); ya no existe el renglón gris "Deshabilitada". Difusión no cuelga de `/asesor-ia-whatsapp` a propósito: `esRutaActiva` toma subrutas y quedarían dos renglones activos. Los títulos del header con acento o sigla salen de `lib/nav/titulos.ts`. Cada renglón declara nombre/href por rol y su grupo (Tracking cambia de grupo por rol: `mi-equipo` para el director, `mi-dia` para el asesor); un grupo sin renglones no se dibuja; ningún icono se repite. `components/sidebar-nav.tsx` dibuja los grupos plegables (todos se pueden cerrar; el grupo activo se abre al navegar; estado en `localStorage` `prisma.menu.abiertos.<rol>`; los contadores de los renglones suben al título del grupo cuando está plegado) y `components/{director,asesor}-sidebar.tsx` solo conservan cabecera, pie (Configuración + logout) y el contador de aprobaciones. **Barra lateral que se cierra en escritorio:** `components/barra-lateral.tsx` (botones) + `lib/nav/barra-lateral.ts` (script inline que lee `localStorage` `prisma.barra` ANTES del primer dibujo y pone `<html data-barra="oculta">`) + reglas en `app/globals.css` (`.barra-escritorio` a `width:0`, `.btn-abrir-barra` visible). El estado no pasa por React a propósito: sin salto de hidratación y sin animación de entrada. Gotcha resuelto: Leaflet no se re-mide al cambiar su caja → `AjustarAlContenedor` en `components/mapa/mapa-lienzo.tsx` (`ResizeObserver` → `invalidateSize`). Headers con `ModeToggle`.
...
- **Componentes compartidos clave:** Pipeline (`PipelineClient`), Tasaciones (wizard en `app/asesor/tasaciones/components/`, reusado por director), Contratos (`components/contratos-ia/ContratosIAPage.tsx`, prop `role`), WhatsApp bot (`components/whatsapp/BandejaWhatsApp.tsx` = solo el chat; las otras secciones son páginas de Difusión dibujadas por `components/difusion/PaginaDifusion.tsx`, que además convierte `CampaignState.setActiveTab('campanas')` —lo que dispara "Configurar Campaña" en `ContactsTab`— en una navegación; `WhatsAppTabsWrapper.tsx` ya no existe). Los saltos automáticos de Marketing IA (`generation-complete` → Historial, `retomar-foto-ia` → HomeStaging) los escucha `components/marketing-ia/navegacion-marketing.tsx` desde el layout de `marketing-ia/`. **Plantillas** (`TemplatesTab`) es CRUD completo (crear/editar/borrar plantillas propias + `syncTemplatesFromMeta`), no solo lectura de las 8 de seguimiento. **Campañas** (`CampaignsTab`) admite importar CSV/Excel (papaparse/xlsx) con mapeo de columnas y variables, además de seleccionar contactos.
...
`esperandoHumano(c)` = `bot_active=false` ∨ `metricas.fue_derivado_a_humano='true'` ∨ `solicito_hablar_con_humano='true'` ∨ `etapa='handoff'`. Caso abierto = último mensaje del lead (`t0`) **sin ningún `wa_messages.role='human'` posterior** (la promesa "ya lo atiendo" por WhatsApp no cierra nada). Niveles desde `t0` **en HORAS HÁBILES** (`horasHabiles` de `lib/whatsapp/sending-window.ts`, 6-23 AR — la noche no corre en contra del asesor; Kevin 2/9): **2 h asesor · 5 h asesor+director · 10 h asesor (`asesor_sigue_esperando`) · 20 h asesor+director**; `nivelQueToca` manda solo el más alto alcanzado y cada nivel una vez por caso (`lead_eventos` tipo `escalera` con `datos.t0` y `datos.nivel`); sin tope por agencia; casos de más de 14 días no cuentan; `casoCuenta(t0, activo_desde)`. **Fuera de 6-23 AR la corrida entera se saltea** (`fueraDeVentana: true` en el resumen). Sin asesor asignado, los avisos van al director. En sombra registra `escalera_simulada`. El aviso suelto del agente al `escalar` se delega a la escalera cuando el lead está esperando a un humano (`aviso_delegado_escalera`). Recordatorios de visita: `visitas.ts` (24 h/3 h/1 h/no-show ≤ 48 h, sin IA; el dispatch ya los gatea 6-23). **Emails de n8n de madrugada** (`Avisar_Asesor`, `Gestion_Handoff`): entre 23 y 6 AR se PROGRAMAN en Resend (`scheduled_at` = próximas 6:00, calculado en el Code node; AR es UTC-3 fijo) — n8n no se bloquea y el anotador de la bitácora dice "programado para las 6:00" (`datos.programado_para`). Respaldos: `scratch/_horario-n8n-RESPALDO-*-2026-09-03.json`.
