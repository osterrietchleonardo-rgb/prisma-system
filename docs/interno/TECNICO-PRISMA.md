...
- `performance_logs` — registro de actividad comercial por asesor (agency_id, agent_id, `type`: prospeccion/prelisting/prebuying/captacion/reserva/cierre, monto_operacion, comision_generada, fecha_actividad, metadata jsonb, `status`: original/modificada/eliminada). Fuente de los KPIs y del Ranking del Dashboard. **Excepción:** la métrica ACM del Ranking ya no sale de acá, sale de `shared_acm_reports` (ver abajo).
...
- **Métrica ACM (jul-2026, reemplaza a "tasaciones"):** la columna "Tasac." del Ranking y la tarjeta "Volumen Tasaciones" del bloque Prelisting dejaron de contar logs `type='prelisting'`. Ahora cuentan **fichas ACM generadas** = filas de `shared_acm_reports` (`created_by`, `agency_id`, `created_at`), es decir el momento en que el asesor realmente arma la ficha, no cuando alguien carga un log a mano. La clave del KPI pasó de `kpis.tasaciones` a `kpis.acm`, y la del ranking de `advisor.tasaciones` a `advisor.acm`.
  ...
  - `pipelineCaptacion`, `ticketPromedioTasacion` y `hitRate` **siguen saliendo de los logs `prelisting`** (son los que tienen el monto). Solo cambió el conteo.
  - El clasificador (`lib/tracking/performance-evaluator.ts`) usa `stats.acm` y el prompt de reglas personalizadas expone la variable como `acm`. **Se sigue mandando también `tasaciones` con el mismo valor** como alias legacy: las reglas que el director ya tenía guardadas la nombran así y se romperían solas. En la UI (`PerformanceScaleEditor`) ya solo se ofrece `acm`.
