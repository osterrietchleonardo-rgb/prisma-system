...
- **Ramas de `createManualContact` según el dueño:** sin match → INSERT normal (con la sesión del asesor, `agent_id` = asesor). Contacto **sin dueño** (típico del webhook, que no graba `agent_id`) → se le asigna al asesor (UPDATE por admin, porque su RLS todavía no lo alcanza): es el momento en que el cliente se engancha con una propiedad. Contacto **de otro asesor** → no se toca ni se le pisan los datos; se reutiliza su `id` y se devuelve `warning` con el nombre del dueño, que el formulario muestra como aviso **sin cortar** el alta. La regla de "no robar" **no aplica al director** (`isDirector`), que sí puede reasignar. Los errores inesperados ya no devuelven `error.message` crudo de Postgres: van a `console.error` y al usuario le llega un mensaje en castellano.
