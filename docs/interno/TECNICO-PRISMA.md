...
**Backfill:** el crawler solo re-procesa propiedades **nuevas o modificadas**, así que las ~129.000 filas ya guardadas siguen cortadas hasta re-visitarlas. `roomix-sync/backfill-descripcion.mjs` las reescribe: reanudable (checkpoint propio), idempotente (siempre reescribe desde el texto que publica Roomix), **solo toca la columna `description`** (no regenera `embedding` ni pisa `lastmod`, clave para el diff del crawler) y tiene `--dry-run` para ver antes/después sin escribir. Por defecto procesa solo las cortadas (≥495 chars); `--todas` incluye las que no estaban cortadas (también les limpia la letra chica). Throughput similar al del catálogo completo (≈1,7 s/ficha a concurrencia 3).
