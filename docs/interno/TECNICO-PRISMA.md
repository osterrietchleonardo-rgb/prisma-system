...
- **Reuso en n8n (jul-2026):** el agente n8n `Cartera_Propiedades` (WhatsApp), cuando el lead pega el link de una propiedad publicada, ya **no** usa el `webSearch` de OpenAI (buscador, no scraper → volvía vacío con ML/ZonaProp). Su herramienta `Informacion_Web` hace `POST /extract` a este mismo servicio pasando la URL ya saneada por el Code node `extraer_agency_id` (`extracted_url`, con strip de puntuación final). ML devuelve datos parciales por JSON-LD (tipo/barrio/amenities, sin precio/ambientes salvo que el extractor tenga `GEMINI_API_KEY` para el fallback IA); alcanza para matchear contra el stock interno por zona+tipo+características. El Code node quedó con `retryOnFail` (3 intentos/2s) para el timeout esporádico del task-runner.
  - **Tipo de nodo (gotcha n8n 2.27.4):** hay que usar `n8n-nodes-base.httpRequestTool` (variante-herramienta del nodo base, typeVersion 4.4, header `x-extractor-secret` inline), **no** el nodo langchain `@n8n/n8n-nodes-langchain.toolHttpRequest`: éste solo tiene `supplyData` (sin `execute`) y el AI Agent v3.1 ejecuta las tools por `runNode` → falla en runtime con *"has a supplyData method but no execute method"* aunque valide en estático. Patrón copiado del `Enviar_Email_Resend` de `Avisar_Asesor`, y mismo criterio que la tool `CARTERA_PROPIEDADES` (postgresTool, variante base).
