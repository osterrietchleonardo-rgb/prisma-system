...
- **Cómputo compartido:** `lib/admin-vakdor/finance/metrics.ts` (`loadFinanceData`, `kpisDeMes`, `estadoResultadoDeMes`, `nAgenciasPagando`) es la **única fuente de verdad**, usada por `finance/metricas` y `finance/analisis`. El **Estado de Resultado** usa un mapeo fijo de categorías (costo de ventas = IA + infra + proxy; operativos = sueldos + marketing + suscripción + otro; financieros = financiero; impuestos = impuestos) con desglose por sub-ítem. El **Punto de equilibrio** se calcula en el cliente (unidad = agencia), prellenado con datos reales y editable. **EBITDA/FCL** (`ebitdaFclDeMes`): EBITDA = utilidad operativa + D&A (categoría `depreciacion`); FCL = EBITDA − impuestos − CAPEX (categoría `capex`) − Δ capital de trabajo (manual, se resta en el cliente y se pasa a `finance/analisis` como `deltaCapitalTrabajo` en USD). El **CAPEX se excluye** de `kpisDeMes` (gastos operativos) y de `categoriaBreakdown` (no es gasto del P&L, es inversión). `metricas` devuelve `ebitdaFcl`; el análisis IA recibe EBITDA y FCL en el resumen.
