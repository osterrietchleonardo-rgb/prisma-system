...
- **Formulario de Registro (PerformanceLogForm):**
  ...
  - **Si el teléfono ya existe en la agencia (jul-2026):** la acción busca el contacto **a nivel agencia** (no solo entre los del asesor) y decide: si no tiene dueño se lo asigna al asesor; si es de otro asesor lo reutiliza **sin modificarlo** y devuelve un `warning` que el formulario muestra como aviso, pero **el registro se guarda igual**. Detalle del porqué en TECNICO § 9.6.
