...
| `match_roomix_ia(p_query_embedding, p_operation, p_type_patterns, p_rooms, p_bedrooms, p_bathrooms, p_price_max/min, p_currency, p_loc_patterns, p_amenity_patterns, p_agency_name_patterns, p_floor_min, p_floor_max, p_free_text_patterns, p_limit)` | **Buscador IA — red de colaboración** (**356.314 filas** al 21/08/2026; la tabla crece, no tomar este número como fijo). El filtro duro corre sobre TODA la base; `hnsw.ef_search` y el pool de candidatos = **1000** (jun-30, antes 400) → de las que pasan el filtro toma las 1000 más parecidas y rankea. Mismo esquema (+ piso suave + free-text). Devuelve `id, match_pct, semantic_sim`. |
...
5. **Si intent === RETRIEVAL — estrategia "Cartera_Propiedades" (paridad con el agente n8n), 2 capas en SQL:**
   ...
   Se llaman 2 funciones SQL (Supabase): **`match_properties_ia`** (cartera propia + agencia; 2 llamadas, `p_include_agent`=propias / `p_exclude_agent`=agencia) y **`match_roomix_ia`** (red de colaboración, sobre **toda** la tabla —356.314 filas al 21/08/2026— **SIN** el viejo límite de 400). Cada una, dentro de la base:
...
6. **Generación de Respuesta y Renderizado Frontend:**
   ...
   - **Enlaces:** propiedades internas → `tokko_data.public_url`; red de colaboración → **el aviso puntual en el portal de origen** (`source_listing_url`), con `canonical_url` (la ficha en roomix.ai, la tienen todas) y `roomix_agency_source_url` (el listado de la inmobiliaria) como respaldos en ese orden. **Hasta ago-2026 el botón sólo aparecía si había `roomix_agency_source_url`**, que le falta al 14% de las filas, así que esas quedaban sin ningún link visible.
   - **Contacto del colega (ago-2026):** las tarjetas de la red muestran el **WhatsApp y el teléfono** de la inmobiliaria que publica (`roomix_properties.phone`/`whatsapp`, cargados en el 81% de las filas), tanto en la tarjeta como en el detalle ("Para coordinar la visita"). Los datos estaban en la base desde siempre y no llegaban a la pantalla: el asesor veía la propiedad y no tenía cómo coordinar la visita. **Esto es interno**: en la ficha pública que se le manda al cliente no va nada del colega (§10.8).
...
- **La ficha de una propiedad de la red NO lleva ningún dato de la inmobiliaria que la publica** (ago-2026): ni el nombre, ni el logo, ni el teléfono, ni el link al aviso original. Es la ficha que el asesor le manda a **su** cliente, y cualquiera de esos datos lo derivaría directo al colega. El contacto del colega existe, pero **solo en la tarjeta del Buscador IA**, que es de uso interno. Decisión de Leonardo.
- **Si la consulta falla, lo dice** (ago-2026): antes un error de base se reportaba como *"No se encontró la propiedad o no pertenece a tu agencia"*, que era falso y sonaba a un problema de permisos. Ahora hay tres respuestas distintas: **503** "probá de nuevo en unos segundos" (la consulta falló), **404** "ya no está publicada por la inmobiliaria que la ofrecía" (propiedad de la red que se dio de baja) y **404** "no se encontró o no pertenece a tu agencia" (solo para la cartera propia). Detalle técnico en TECNICO-PRISMA §10.1.c.
...
**Qué muestra:** KPIs de P&L (Ingresos, Costos de IA, Gastos fijos/variables, Margen de contribución, Utilidad EBIT, Margen neto, **Apalancamiento operativo = MC/EBIT**), evolución mensual ingresos vs costos (12 meses), tortas de costo por proveedor y gasto por categoría, **Estado de Resultado contable clásico** (ventas → costo de ventas → utilidad bruta → gastos operativos → utilidad operativa → gastos financieros → utilidad antes de impuestos → impuestos → utilidad neta, con sub-renglones desglosados y % sobre ventas), **EBITDA y Flujo de Caja Libre** (puente utilidad operativa → EBITDA → FCL con márgenes), **Punto de equilibrio** (en cantidad de agencias, prellenado con datos reales y editable para simular), **Análisis del experto IA** (Gemini), tabla de gastos con alta/edición/baja, y toggle **USD/ARS**. Un botón de info (ⓘ) junto al título abre una **guía embebida** que explica cada término (Estado de Resultado, márgenes, EBITDA, punto de equilibrio, FCL) aplicado al negocio, con "qué mirar / cómo analizarlo" y una rutina mensual.
...
**EBITDA y Flujo de Caja Libre (FCL):** **EBITDA** = utilidad operativa + **Depreciación/Amortización** (categoría de gasto nueva; se vuelve a sumar porque no es salida de caja). **FCL** = EBITDA − impuestos − **CAPEX** (categoría "Inversiones/CAPEX" nueva) − **variación de capital de trabajo** (calculada: se cargan los saldos mensuales de 4 partidas —cuentas por cobrar, por pagar, anticipos de clientes, gastos pagados por adelantado— en `finance_working_capital` y el sistema calcula el Δ contra el mes anterior; por cobrar/prepago inmovilizan caja, por pagar/anticipos la liberan). Importante: el **CAPEX no es gasto del Estado de Resultado** (es inversión) → se excluye del EBIT, de los KPIs de gastos y de la torta de categorías; solo impacta el FCL. La UI muestra el "puente" (utilidad operativa → +D&A → EBITDA → −impuestos → −CAPEX → −Δ capital de trabajo → FCL) con margen EBITDA% y FCL% sobre ventas. EBITDA y FCL también se le pasan al experto IA para que los comente.
...
- **Vista Pipeline (tablero por etapas, jul-2026):** dentro de la solapa *Actividad* hay un switch **Lista | Pipeline** (al lado del filtro de fechas). El tablero muestra **un cliente = una tarjeta**, parado en su etapa actual, y se puede mover entre las 6 columnas (`prospeccion → prelisting → prebuying → captacion → reserva → cierre`, orden lineal fijo). Archivos: `lib/tracking/pipeline.ts` (lógica pura, sin I/O), `components/tracking/pipeline/*` (`PipelineBoard`, `PipelineColumn`, `PipelineCard`, `PipelineStageDialog`, `PipelineClientSheet`), `actions/tracking/movePipelineCard.ts`.
  ...
  - **Un cliente, hasta cuatro procesos (ago-2026, ampliado a cuatro valores 20-ago-2026):** la unidad del tablero es **(cliente, proceso)**. Columna `proceso` (`'vendedor' | 'comprador' | 'locador' | 'locatario' | null`) en `performance_logs` y en `tracking_pipeline_moves`, nullable en ambas (`NULL` = "sin definir": solo lo tienen filas históricas que el backfill no pudo resolver; toda alta nueva lo exige). Vocabulario puro en `lib/tracking/proceso.ts` (`ProcesoNegocio`, `PROCESOS_POR_ETAPA`, `ladoDelNegocio`, `procesosPosiblesParaTarjeta`), separado de `pipeline.ts` a propósito porque los server actions no tienen por qué arrastrar los imports de íconos que trae ese archivo. Origen: sugerencia de un asesor de Central Real Estate Argentina; ampliado de dos a cuatro valores el 20-ago-2026 porque el formulario ya tenía un campo "Tipo de Lead" (Vendedor/Comprador/Locador/Locatario) que solo cubría Prospección — ver el addendum al final de `docs/superpowers/specs/2026-08-19-tracking-cliente-doble-proceso-design.md`.
    - **El lado del negocio se deriva, ninguna etapa fuerza un solo valor:** `ladoDelNegocio(proceso)` devuelve `'ofrece'` (vendedor, locador) o `'busca'` (comprador, locatario); ya no se guarda como valor propio. `PROCESOS_POR_ETAPA` (reemplaza al viejo `PROCESO_FIJO`, que fijaba un único valor) define qué admite cada etapa: Prelisting y Captación → `vendedor` o `locador`; Prebuying → `comprador` o `locatario`; Prospección, Reserva y Cierre → los cuatro. En `PerformanceLogForm` el asesor contesta esta pregunta en las **seis** etapas; el campo solo viene relleno y deshabilitado cuando el `proceso` lo impone el popup del tablero (mover una tarjeta, o abrir un proceso adicional desde la ficha del cliente — prop `forcedProceso`), nunca por ser una etapa en particular. El `CHECK` en `performance_logs` se reemplazó en la migración `20260820120000_proceso_cuatro_valores.sql` para validar contra la lista de cada etapa (por ejemplo, rechaza un Prelisting con `proceso = 'comprador'` o `'locatario'`).
    - **`cardKey` sigue siendo `${clientKey}::${proceso ?? 'sin-definir'}`, ahora sobre cuatro valores posibles:** un cliente puede terminar con **hasta cuatro tarjetas independientes** (una por `vendedor`, `comprador`, `locador` y `locatario` con actividad propia — no solo dos como en la versión original), cada una parada en la etapa de su propio evento más reciente y avanzando sola. `clientKey` no cambia (lo siguen usando `movePipelineCard` y la ficha del cliente); el mapa de "último movimiento manual" en `buildPipeline` sigue indexado por `client_key + proceso`.
    - **Restricción al arrastrar (`ETAPAS_POR_PROCESO`, derivado del lado):** tarjeta `vendedor` o `locador` (lado 'ofrece') entra en todas las columnas menos Prebuying; tarjeta `comprador` o `locatario` (lado 'busca') entra en todas menos Prelisting/Captación; tarjeta `sin-definir` entra en todas, sin cambios respecto al comportamiento anterior. Un destino no permitido hace que la tarjeta vuelva sola a su lugar con un toast explicando el motivo. El mismo bloqueo se aplica al menú "Mover a…" de `PipelineCard`.
    - **Abrir un proceso adicional no mueve la tarjeta existente** — sigue siendo el objetivo del mecanismo. El botón "Abrir proceso de…" en `PipelineClientSheet` ofrece los valores que ese cliente todavía no tiene (hasta tres botones, uno por cada `ProcesoNegocio` que le falte) y abre `PerformanceLogForm` con el cliente ya fijado (`lockedClient`) y la etapa de arranque según el lado derivado (`ladoDelNegocio(proceso) === "ofrece" ? "prelisting" : "prebuying"` en `PipelineBoard.tsx`): siempre Prelisting para 'ofrece' (nunca Captación, aunque el asesor pueda cambiarla después con el dropdown), Prebuying para 'busca'.
    - **`PipelineClientSheet` muestra el historial de su propia tarjeta únicamente** (cada proceso tiene su propia historia y sus propios botones de editar). En el encabezado avisa de los demás procesos que el cliente ya tiene abiertos (ej. "También tiene un proceso de Locador abierto").
    - **Cartelito de proceso** en `PipelineCard` y en `PerformanceHistoryList` (vista Lista), cuatro valores: el color comunica el lado y el texto el valor exacto — VENDEDOR y LOCADOR en índigo (`indigo-500`, lado 'ofrece'), COMPRADOR y LOCATARIO en violeta (`violet-500`, lado 'busca'), SIN DEFINIR en ámbar (`amber-500`).
    - **El resolutor de "Sin definir"** (en `PipelineClientSheet`, función `procesosPosiblesParaTarjeta`) ofrece solo los valores que sirven para **todas** las actividades de esa tarjeta (la intersección de `PROCESOS_POR_ETAPA` entre sus `type`). Si la tarjeta mezcla actividades de los dos lados (por ejemplo una Prelisting y una Prebuying nunca clasificadas) la intersección queda vacía y el panel explica que hay que resolverla actividad por actividad desde la vista Lista. Clasificar no marca las filas como "Modificada" ni les toca la calificación de IA (server action `asignarProcesoATarjeta`).
    - **El campo "Tipo de Lead" se sacó de Prospección** (`PerformanceLogForm.tsx`, bloque `activityType === "prospeccion"`): era la pregunta duplicada que `proceso` absorbió. Los valores históricos en `metadata.tipo_lead` **no se borraron** — se ven igual que cualquier otro campo de `metadata` en la fila expandida de `PerformanceHistoryList`, pero ya no se escriben en altas nuevas.
    - **No toca las demás métricas del pipeline:** verificado en `lib/queries/dashboard.ts` — el embudo, tasas y GCI filtran por `type`, ninguna agrupa por cliente ni por tarjeta. Lo que sí cambia dentro de ese archivo: los contadores de "leads por tipo" dejaron de leer `metadata.tipo_lead` (que solo existía en Prospección y no cubría alquileres) y ahora leen `proceso` directo, sumando **cuatro** buckets (`leadsVendedores`, `leadsCompradores`, `leadsLocadores`, `leadsLocatarios`). El tile "Composición Demanda" (`PerformanceMetricsGrid.tsx`) sigue mostrando `${V}V / ${C}C` mientras la agencia no tenga alquileres cargados, y agrega `/ ${L}L / ${Lt}Lt` en cuanto aparece el primero.
