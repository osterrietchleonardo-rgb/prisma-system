...
5. **Bot IA prendido/apagado por campaña:** al crear, el director elige con un interruptor si los chats que cree la campaña nacen con la IA **prendida** (clientes) o **apagada** (reclutamiento u otros no-clientes; el chat queda en modo manual y la IA no responde). Se guarda en `wa_campaigns.bot_active_on_reply` (default `true`). El webhook ya respeta `bot_active` (si está OFF, guarda el mensaje del lead pero no dispara la IA).
   - **Ojo (cambio jul-2026):** desde el fix de duplicación, la campaña aplica su `bot_active_on_reply` **y su clasificación** también a los chats que **ya existían**, no solo a los nuevos. Es a propósito (alinea el chat con la campaña en curso), pero tiene un costo: **pisa** el interruptor que el asesor haya puesto a mano y **reemplaza** la clasificación previa del lead. Si un comprador viejo entra en una campaña de reclutamiento, queda reclasificado como tal.
