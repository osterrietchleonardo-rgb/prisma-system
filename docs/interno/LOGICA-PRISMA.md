...
- **Vista Pipeline (tablero por etapas, jul-2026):** dentro de la solapa *Actividad* hay un switch **Lista | Pipeline** (al lado del filtro de fechas). El tablero muestra **un cliente = una tarjeta**, parado en su etapa actual, y se puede mover entre las 6 columnas (`prospeccion → prelisting → prebuying → captacion → reserva → cierre`, orden lineal fijo). Archivos: `lib/tracking/pipeline.ts` (lógica pura, sin I/O), `components/tracking/pipeline/*` (`PipelineBoard`, `PipelineColumn`, `PipelineCard`, `PipelineStageDialog`, `PipelineClientSheet`), `actions/tracking/movePipelineCard.ts`.
  ...
  - **Un cliente, dos procesos (ago-2026):** la unidad del tablero deja de ser el cliente y pasa a ser **(cliente, proceso)**. Columna nueva `proceso` (`'compra' | 'venta' | null`) en `performance_logs` y en `tracking_pipeline_moves`, nullable en ambas (`NULL` = "sin definir": solo lo tienen filas históricas que el backfill de la migración no pudo resolver; toda alta nueva lo exige). Vocabulario puro en `lib/tracking/proceso.ts` (`ProcesoNegocio`, `PROCESO_FIJO`, `ETAPAS_POR_PROCESO`), separado de `pipeline.ts` a propósito porque los server actions no tienen por qué arrastrar los imports de íconos que trae ese archivo. Origen: sugerencia de un asesor de Central Real Estate Argentina — ver `docs/superpowers/specs/2026-08-19-tracking-cliente-doble-proceso-design.md`.
    - **Etapas con proceso fijo:** Prelisting y Captación → `venta`; Prebuying → `compra` (mapa `PROCESO_FIJO`). En `PerformanceLogForm` el campo se muestra relleno y deshabilitado, con la leyenda del motivo (una "captación de compra" no es un caso de uso válido). Prospección, Reserva y Cierre lo dejan a elección del asesor, sin valor por defecto. Al cambiar el tipo de actividad el campo **se limpia** incondicionalmente (`setValue("proceso", undefined)`) y solo **se vuelve a completar** si la nueva etapa es una de las tres que lo fijan; si no, queda vacío a propósito, para que nunca quede seleccionado por inercia de la etapa anterior. Un `CHECK` en `performance_logs` blinda la coherencia a nivel base (migración `20260819140000_add_proceso_a_tracking.sql`), para que ni la API ni un `UPDATE` directo puedan guardar, por ejemplo, un Prelisting con `proceso = 'compra'`.
    - **`cardKey` reemplaza al agrupado por cliente puro:** `cardKey = ${clientKey}::${proceso ?? 'sin-definir'}`. Un cliente con actividad de compra **y** de venta genera **dos tarjetas independientes**, cada una parada en la etapa de su propio evento más reciente y avanzando sola; un cliente con un solo lado sigue generando una sola tarjeta, igual que antes. `clientKey` no cambia (lo siguen usando `movePipelineCard` y la ficha del cliente); lo nuevo es que el mapa de "último movimiento manual" en `buildPipeline` pasa a indexar por `client_key + proceso` — indexado solo por cliente, arrastrar la tarjeta de Compra habría movido también la de Venta.
    - **Restricción al arrastrar (`ETAPAS_POR_PROCESO`):** tarjeta `venta` entra en todas las columnas menos Prebuying; tarjeta `compra` entra en todas menos Prelisting/Captación; tarjeta `sin-definir` entra en todas, sin cambios respecto al comportamiento anterior. Un destino no permitido hace que la tarjeta vuelva sola a su lugar (se recalcula desde los datos, igual que ya pasaba con cualquier movimiento) con un toast explicando el motivo, no un error genérico. El mismo bloqueo se aplica al menú "Mover a…" de `PipelineCard`: las etapas del otro lado quedan deshabilitadas.
    - **Abrir el segundo proceso no mueve la tarjeta existente** — es el objetivo del cambio. Dos caminos, mismo formulario: "Nueva Actividad" (elegir cliente + proceso) o el botón "Abrir proceso de Compra/Venta" en `PipelineClientSheet`, que abre `PerformanceLogForm` con el cliente ya fijado (`lockedClient`) y la etapa de arranque de ese lado (Prelisting para venta, Prebuying para compra).
    - **`PipelineClientSheet` muestra el historial de su propia tarjeta únicamente** (no mezcla los dos procesos: cada uno tiene su propia historia y sus propios botones de editar). En el encabezado avisa si el cliente tiene el otro proceso abierto ("También tiene un proceso de Venta abierto").
    - **Cartelito de proceso** en `PipelineCard` y en `PerformanceHistoryList` (vista Lista): COMPRA violeta (`violet-500`, el de Prebuying), VENTA índigo (`indigo-500`, el de Prelisting), SIN DEFINIR ámbar (`amber-500`).
    - **No toca métricas:** verificado en `lib/queries/dashboard.ts` — todas las métricas filtran por `type`, ninguna agrupa por cliente ni por tarjeta. El cambio es puramente aditivo sobre Dashboard, Objetivos e Informe semanal del director.
