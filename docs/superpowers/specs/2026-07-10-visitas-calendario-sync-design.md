...
- **Fase 2 — Confirmación por el cliente (WhatsApp). IMPLEMENTADA (2026-07-10).** El nodo `Analizar_Conversacion2` (workflow `PRISMA`) ya interpreta el chat y detecta la confirmación explícita (`visit_status='confirmed'`). Se agregó el nodo `Confirmar_Visita_Calendario` (paralelo a `Actualizar_Metricas2`) que, cuando el LLM detecta confirmación, hace `UPDATE scheduled_visits SET estado_visita='confirmada'` en la visita activa (`agendada`/`reprogramada`) del teléfono+agencia. El trigger propaga a `visit_status='confirmed'` y **se refleja en el calendario del asesor**. Como corre dentro de `PRISMA`, aplica con bot activo; con bot apagado, la confirmación la marca el asesor con el botón de la UI (Fase 3). Calendario = fuente única.
- **Fase 3 — UI del calendario + `realizada` automática. IMPLEMENTADA (2026-07-10).** (a) Componente compartido `components/calendar/VisitStatusActions.tsx` con botones Confirmar asistencia / Marcar realizada / No asistió, usado en `app/director/calendario` y `app/asesor/calendario`; escribe `scheduled_visits.estado_visita` (fuente de verdad) + `triggerCalendarSync`. (b) Nodo `Auto_Realizada` en el workflow `Seguimiento` (colgado de `Reloj - 30min`): `UPDATE scheduled_visits SET estado_visita='realizada' WHERE estado_visita='confirmada' AND (fecha+hora) AT TIME ZONE AR < now()`. (c) Precedencia del asesor: el auto solo transiciona `confirmada`→`realizada`; el botón "No asistió" (disponible desde `confirmada` y desde `realizada`) siempre pisa lo automático.
